...
from diff_gaussian_rasterization_contrastive_f import GaussianRasterizationSettings as Camera2
...
def setup_camera(w, h, k, w2c, near=0.01, far=100):
    ...
    camera_parameters = {
        ...
        'projection_matrix_as_pytorch':opengl_proj.squeeze(0),
        'projmatrix' : full_proj,
        'sh_degree' : 0,
        'campos' : cam_center,
        'prefiltered' : False}
...
def setup_camera2(w, h, k, w2c, near=0.01, far=100):
    fx, fy, cx, cy = k[0][0], k[1][1], k[0][2], k[1][2]
    w2c = torch.tensor(w2c).cuda().float()
    cam_center = torch.inverse(w2c)[:3, 3]
    w2c = w2c.unsqueeze(0).transpose(1, 2)
