...
class GaussRenderer(nn.Module):
    ...
    def __init__(self, active_sh_degree=0, white_bkgd=False, original_im = None,**kwargs):
        super(GaussRenderer, self).__init__()
        self.active_sh_degree = active_sh_degree
        self.debug = False
        self.white_bkgd = white_bkgd
        if original_im is not None:
            c,h,w =original_im.shape
        else:
            print('Error: original_im is None')
        #self.pix_coord = torch.stack(torch.meshgrid(torch.arange(256), torch.arange(256), indexing='xy'), dim=-1).to('cuda')# Nirit:I think I need to change this to my size
        self.pix_coord = torch.stack(torch.meshgrid(torch.arange(w), torch.arange(h), indexing='xy'), dim=-1).to('cuda')
    ...
    def render(self, camera, means2D, cov2d, color, opacity, features,depths):
        ...
        self.render_feature = torch.zeros(*self.pix_coord.shape[:2], features.shape[1]).to('cuda')

        TILE_SIZE = 16
        for h in range(0, camera['image_height'], TILE_SIZE):
            for w in range(0, camera['image_width'], TILE_SIZE):
                # check if the rectangle penetrate the tile
                clip_max_val = min(w+TILE_SIZE-1, camera['image_width']-1),min(h+TILE_SIZE-1, camera['image_height']-1)
                over_tl = rect[0][..., 0].clip(min=w), rect[0][..., 1].clip(min=h)
                over_br = rect[1][..., 0].clip(max = clip_max_val[0]), rect[1][..., 1].clip(max = clip_max_val[1])
                ...
                P = in_mask.sum()
                tile_coord = self.pix_coord[h:h+TILE_SIZE, w:w+TILE_SIZE].flatten(0,-2)#If it is bigger than the actual size it truncate 
                ...
                sorted_feature = features[in_mask][index]
                ...
                #Nirit: I think I need to change this because ther is a bug!
                current_tile_shape = self.pix_coord[h:h+TILE_SIZE, w:w+TILE_SIZE].shape[:2]
                if current_tile_shape[0] != TILE_SIZE or current_tile_shape[1] != TILE_SIZE:
                    h_new, w_new = current_tile_shape
                    self.render_color[h:h+h_new, w:w+w_new] = tile_color.reshape(h_new, w_new, -1)
                    self.render_depth[h:h+h_new, w:w+w_new] = tile_depth.reshape(h_new, w_new, -1)
                    self.render_feature[h:h+h_new, w:w+w_new] = tile_feature.reshape(h_new, w_new, -1)
                    self.render_alpha[h:h+h_new, w:w+w_new] = acc_alpha.reshape(h_new, w_new, -1)
                else:   
                    ...
                    self.render_feature[h:h+TILE_SIZE, w:w+TILE_SIZE] = tile_feature.reshape(TILE_SIZE, TILE_SIZE, -1)
                    self.render_alpha[h:h+TILE_SIZE, w:w+TILE_SIZE] = acc_alpha.reshape(TILE_SIZE, TILE_SIZE, -1)
                #mine:
                #self.render_color[w:w+TILE_SIZE, h:h+TILE_SIZE] = tile_color.reshape(TILE_SIZE, TILE_SIZE, -1)
                #self.render_depth[w:w+TILE_SIZE, h:h+TILE_SIZE] = tile_depth.reshape(TILE_SIZE, TILE_SIZE, -1)
                #self.render_feature[w:w+TILE_SIZE, h:h+TILE_SIZE] = tile_feature.reshape(TILE_SIZE, TILE_SIZE, -1)
                #self.render_alpha[w:w+TILE_SIZE, h:h+TILE_SIZE] = acc_alpha.reshape(TILE_SIZE, TILE_SIZE, -1)
    ...
    def forward(self, camera, pc, **kwargs):
        ...
        features_all = pc.get_features
        color = features_all[:, :3]
        features = features_all[:, 3:]
        ...
        with prof("projection"):
            mean_ndc, mean_view, in_mask = projection_ndc(means3D, 
                    ...
                    projmatrix=camera['projection_matrix_as_pytorch'])
            depths = mean_view[:,2]
        ...
        with prof("build cov2d"):
            cov2d = build_covariance_2d(
                ...
                cov3d=cov3d, 
                viewmatrix=camera['viewmatrix'],
                fov_x=2*math.atan(camera['tanfovx']), 
                fov_y=2*math.atan(camera['tanfovy']), 
