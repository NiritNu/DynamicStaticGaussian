...
class GaussRenderer(nn.Module):
    ...
    def __init__(self, original_im, active_sh_degree=3, white_bkgd=True,**kwargs):
        super(GaussRenderer, self).__init__()
        self.active_sh_degree = active_sh_degree
        self.debug = False
        self.white_bkgd = white_bkgd
        h,w,c =original_im.shape
        #self.pix_coord = torch.stack(torch.meshgrid(torch.arange(256), torch.arange(256), indexing='xy'), dim=-1).to('cuda')# I think I need to change this to my size
        self.pix_coord = torch.stack(torch.meshgrid(torch.arange(h), torch.arange(w), indexing='xy'), dim=-1).to('cuda')
    ...
    def render(self, camera, means2D, cov2d, color, opacity, features,depths):
        ...
        for h in range(0, camera['image_height'], TILE_SIZE):
            for w in range(0, camera['image_width'], TILE_SIZE):
                ...
                #Nirit : I assume it is already in the right order since I dont have depth ...may be a source for a bug
